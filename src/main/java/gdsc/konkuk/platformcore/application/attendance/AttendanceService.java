...
public class AttendanceService {
    ...
    public List<Attendance> getAllByPeriod(LocalDate month) {
        return attendanceRepository.findAllByPeriod(month.atStartOfDay(),
                month.atStartOfDay().plusMonths(1));
    }
    ...
    public AttendanceStatus getAttendanceStatus(Long attendanceId) {
        List<Participant> totalParticipants = participantRepository.findAllByAttendanceId(
                attendanceId);
        List<Participant> attendedParticipants = totalParticipants.stream()
                .filter(Participant::isAttend).toList();
        return AttendanceStatus.of(attendanceId, totalParticipants.size(),
                attendedParticipants.size());
    }
