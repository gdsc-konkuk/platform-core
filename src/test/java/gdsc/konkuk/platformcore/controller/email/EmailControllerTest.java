...
class EmailControllerTest {
  ...
  void should_success_when_send_email() throws Exception {
    ...
    // when
    when(emailService.registerTask(any())).thenReturn(mockTask);
            ...
            resource(
                ResourceSnippetParameters.builder()
                    ...
                    .requestFields(
                        ...
                        fieldWithPath("receivers")
                            .type(JsonFieldType.ARRAY)
                            .description("수신자 이메일 목록"),
                        fieldWithPath("sendAt")
  ...
  void should_success_when_update_emailTask() throws Exception {
    ...
    ResultActions result =
        mockMvc.perform(
            RestDocumentationRequestBuilders.put("/api/v1/emails/1")
                .contentType(APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request))
                .with(csrf()));
  ...
  void should_success_when_get_all_task() throws Exception {
    ...
    EmailReceivers emailReceivers =
        new EmailReceivers(
            List.of("example1@gmail.com", "example2@gmail.com", "example3@gmail.com"));
    EmailTask emailTask =
        new EmailTask(1L, emailDetails, emailReceivers, LocalDateTime.of(2024, 7, 20, 12, 30));
            ...
            resource(
                ResourceSnippetParameters.builder()
                    ...
                    .responseFields(
                        ...
                        fieldWithPath("data.emailTasks[].id")
                            .description("이메일 작업의 ID (Mock객체에 대해 null일 수 있음.)"),
                        fieldWithPath("data.emailTasks[].subject").description("이메일 제목"),
                        fieldWithPath("data.emailTasks[].receiver").description("이메일 수신자"),
                        fieldWithPath("data.emailTasks[].sendAt")
                            .description("이메일 발송 예정 시간 (ISO 8601 형식)"),
  ...
  void should_success_when_get_specific_task() throws Exception {
    ...
    EmailDetails emailDetails = new EmailDetails("예시 이메일 제목", "Html 문자열");
    EmailReceivers emailReceivers =
        new EmailReceivers(List.of("example@gmail.com", "example@naver.com"));
    EmailTask emailTask =
        new EmailTask(1L, emailDetails, emailReceivers, LocalDateTime.of(2024, 7, 20, 12, 30));
        ...
        document(
            ...
            resource(
                ResourceSnippetParameters.builder()
                    ...
                    .pathParameters(parameterWithName("emailId").description("취소할 이메일 작업 ID"))
                    .build())));
  }
  ...
  void should_success_when_cancel_registered_task() throws Exception {
    // given
    doNothing().when(emailService).delete(any());
